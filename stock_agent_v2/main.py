...
def run_morning_brief():
    ...
    try:
        ...
        # ── 자연어 시그널 (저장된 NL 시그널 자동 실행) ─────────────────
        # 매치는 signals 테이블에 저장 → 웹 /signals, /screener 에서 확인.
        # 텔레그램은 모닝 브리핑 1통이면 충분 — 발동 종목 한 줄 요약만 첨부.
        try:
            from nl_screener import run_morning_nl_signals
            summary = run_morning_nl_signals()
            if summary["runs"]:
                lines = ["", "🔍 NL 시그널 자동 실행"]
                for r in summary["runs"]:
                    if "error" in r:
                        ...
                        continue
                    cnt = r.get("matched_count", 0)
                    if cnt == 0:
                        lines.append(f"  · {r['name']} ({r['scope']}): 매치 없음")
                    else:
                        tks = ", ".join(m["ticker"] for m in r.get("matches", [])[:6])
                        more = f" 외 {cnt - 6}건" if cnt > 6 else ""
                        lines.append(f"  · {r['name']} ({r['scope']}): {cnt}건 — {tks}{more}")
                tail = "\n".join(lines)
                try:
                    TelegramNotifier(
...
def run_kr_evening():
    ...
    try:
        ...
        print(f"[ETF] 체이닝 실행 실패 (evening 완료): {e}")
...
if __name__ == "__main__":
    ...
    if   "--init"      in args: cmd_init()
        ...
        # 스케줄 잡은 Windows Task Scheduler 가 담당. run_daily.py 가 진입점.
        # 텔레그램 명령은 telegram_trigger.py 가 listener.
        print("Usage: python main.py [--init|--update|--analyze|--dart-only|--weekly|--signals] [--ticker X]")
        print("Daily batches run via Windows Task Scheduler → run_daily.py morning/evening")
        sys.exit(2)
